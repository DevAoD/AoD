...
void DividendDistributor::GenerateOutputs(int nTransactions, vector<Object> &vTransactionOuts) const
{
    ...
    BOOST_FOREACH(const Distribution &distribution, vDistribution)
    {
        ...
        string address = distribution.GetAoDcoinAddress().ToString();
...
DividendDistributor GenerateDistribution(const BalanceMap &mapBalance, double dAmount)
{
    ...
    printf("Distributing %f AoDcoins to %d addresses with a minimum payout of %f\n", dAmount, mapBalance.size(), dMinPayout);
...
{
    // As of 2014-02-22, AoDcoin won't generate transactions larger than 100,000 bytes (MAX_BLOCK_SIZE_GEN/5)
    // https://github.com/AoD/AoD/blob/master/src/wallet.cpp#L1181
...
Array SendDistribution(const DividendDistributor &distributor)
{
    try {
        ...
        if (dTotalDistributed > dBalance)
            throw runtime_error("Not enough AoDcoins available in distribution account");
        ...
        printf("Will send %f AoDcoins to %d addresses in %d transactions\n", dTotalDistributed, nDistributionCount, nTransactions);
        ...
        BOOST_FOREACH(Object &output, vOutputs)
        {
            ...
            result = CallAoDcoinRPC("sendmany", sendmanyParams);
...
double GetDistributionBalance()
{
    ...
    return boost::lexical_cast<double>(Value(CallAoDcoinRPC("getbalance", params)).get_str());
}
