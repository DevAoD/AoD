...
BOOST_AUTO_TEST_CASE( test_simple_distribution )
{
    ...
    BOOST_CHECK_CLOSE(25.0, find_value(outs, CAoDcoinAddress(1).ToString()).get_real(), PRECISION);
    BOOST_CHECK_CLOSE(75.0, find_value(outs, CAoDcoinAddress(2).ToString()).get_real(), PRECISION);
}
...
BOOST_AUTO_TEST_CASE( test_split_transaction )
{
    ...
    outs = *iTransaction;
    BOOST_CHECK_CLOSE(1.0, find_value(outs, CAoDcoinAddress(1).ToString()).get_real(), PRECISION);
    BOOST_CHECK_CLOSE(8.0, find_value(outs, CAoDcoinAddress(2).ToString()).get_real(), PRECISION);
    BOOST_CHECK_CLOSE(1.0, find_value(outs, CAoDcoinAddress(3).ToString()).get_real(), PRECISION);
