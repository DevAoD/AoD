...
// Copyright (c) 2011-2013 The AoDcoin developers
// Copyright (c) 2013-2014 The AoD developers
...
// Global state
//
static const uint256 hashGenesisBlockOfficial("0x000005c98e548676e9f88bc103ed196fed7cbf786ae74030f909cbd7249fe863");
static const uint256 hashGenesisBlockTestNet ("0x000005c98e548676e9f88bc103ed196fed7cbf786ae74030f909cbd7249fe863");
...
const string strMessageMagic = "AoD Signed Message:\n";
...
void static SyncWithWallets(const CTransaction& tx, const CBlock* pblock = NULL, bool fUpdate = false, bool fConnect = true)
{
    if (!fConnect)
    {
        // AoD: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // AoD: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // AoDcoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// AoDcoin: find block wanted by given orphan block
...
// AoD: minter's coin stake is rewarded based on coin age spent (coin-days)
...
// AoDcoin: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // AoDcoin: target change every block
    // AoDcoin: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // AoDcoin: should not enter safe mode for longer invalid chain
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // AoDcoin: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // AoDcoin: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // AoDcoin: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // AoDcoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // AoDcoin: track money supply and mint amount info
    ...
    // AoDcoin: fees are not collected by miners as in bitcoin
    // AoDcoin: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// AoDcoin: total coin age spent in transaction, in the unit of coin-days.
...
// AoDcoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // AoDcoin: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // AoDcoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // AoDcoin: record proof-of-stake hash value
    ...
    // AoDcoin: compute stake modifier
...
bool CBlock::CheckBlock() const
{
    ...
    // AoDcoin: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // AoDcoin: coinbase output should be empty if proof-of-stake block
...
bool CBlock::AcceptBlock()
{
    ...
    // AoD: check switch from proof of work to proof of stake
    ...
    // AoDcoin: check that the block satisfies synchronized checkpoint
    ...
    // AoDcoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // AoDcoin: verify hash target and signature of coinstake tx
    ...
    // AoDcoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // AoDcoin: getblocks may not obtain the ancestor block rejected
    ...
    // AoDcoin: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "AoD", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "AoD", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "2016 July AoD - Public Coin";
        unsigned int nTimeGenesis=1467900019;
        unsigned int nNonceGenesis=386157;

        if (fTestNet)
        {
            pszTimestamp="2016 July AoD - Public Coin";
            nTimeGenesis=1467900019;
            nNonceGenesis=386157;
        }
        ...
        printf("AoD Genesis Block Found:\n");
        printf("genesis hash=%s\n", block.GetHash().ToString().c_str());
        printf("merkle root=%s\n", block.hashMerkleRoot.ToString().c_str());
        block.print();
     
        printf("End AoD Genesis Block\n");
        ...
        printf("%s\n", block.hashMerkleRoot.ToString().c_str());
        if (!fTestNet)
            assert(block.hashMerkleRoot == uint256("0xedb5de33231731161e5d8691da07f837b4e557caa3aaee31b4c585b77a702b79"));
        else
            assert(block.hashMerkleRoot == uint256("0xedb5de33231731161e5d8691da07f837b4e557caa3aaee31b4c585b77a702b79"));
        ...
        // AoDcoin: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");
    }

    // AoDcoin: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // AoDcoin: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // AoDcoin: should not enter safe mode for longer invalid chain
    // AoDcoin: if sync-checkpoint is too old do not enter safe mode
    if (Checkpoints::IsSyncCheckpointTooOld(60 * 60 * 24 * 10) && !fTestNet)
    {
        nPriority = 100;
        strStatusBar = "WARNING: Checkpoint is too old. Please wait for the block chain to download, or notify developers of the issue (https://github.com/AoD/AoD/issues).";
    }

    // AoDcoin: if detected invalid checkpoint enter safe mode
    if (Checkpoints::hashInvalidCheckpoint != 0)
    {
        nPriority = 3000;
        strStatusBar = strRPC = "WARNING: Invalid checkpoint found! Displayed transactions may not be correct! You may need to upgrade, or notify developers of the issue (https://github.com/AoD/AoD/issues).";
    }
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // AoDcoin: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // AoDcoin: record my external IP reported by peer
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (!fAlreadyHave)
                pfrom->AskFor(inv, IsInitialBlockDownload()); // AoD: immediate retry during initial download
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // AoDcoin: send latest proof-of-work block to allow the
                        // download node to accept as orphan (proof-of-stake 
                        // block might be rejected by stake connection check)
                        // AoD: send latest block
                        vector<CInv> vInv;
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // AoDcoin: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // AoDcoin: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // AoDcoin: simplify transaction fee - allow free = false
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // AoDcoin: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
