...
// AoDcoin: optional setting to unlock wallet for block minting only;
...
// AoDcoin: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // AoDcoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // AoDcoin: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar", true)) // AoDcoin: not avatar mode; AoD: avatar mode enabled by default to avoid change being sent to hidden address
                    {
...
// AoDcoin: create coin stake transaction
...
string CWallet::SendMoney(CScript scriptPubKey, int64 nValue, CWalletTx& wtxNew, bool fAskFee)
{
    ...
    if (!CommitTransaction(wtxNew, reservekey))
        return _("Error: The transaction was rejected.  This might happen if some of the shares in your wallet were already spent, such as if you used a copy of wallet.dat and shares were spent in the copy but not marked as spent here.");
...
// AoDcoin: check 'spent' consistency between wallet and txindex
// AoDcoin: fix wallet spent state according to txindex
...
// AoDcoin: disable transaction (only for coinstake)
...
void CWallet::ExportAoDcoinKeys(int &nExportedCount, int &nErrorCount)
{
    nExportedCount = 0;
    nErrorCount = 0;
    ...
    BOOST_FOREACH(const PAIRTYPE(CBitcoinAddress, string)& item, mapAddressBook)
    {
        ...
        if (address.IsScript())
        {
            ...
            json_spirit::Array vAoDcoinAddressStrings;
            BOOST_FOREACH(const CBitcoinAddress &address, vAddresses)
                vAoDcoinAddressStrings.push_back(CAoDcoinAddress(address).ToString());

            json_spirit::Array params;
            params.push_back(json_spirit::Value(nRequired));
            params.push_back(vAoDcoinAddressStrings);
            params.push_back("AoD");

            try
            {
                string result = CallAoDcoinRPC("addmultisigaddress", params);
                printf("Exported multisig address %s: %s\n", address.ToString().c_str(), result.c_str());
                nExportedCount++;
            }
            catch (AoDcoin_rpc_error &error)
            {
        ...
        else
        {
            ...
            json_spirit::Array params;
            params.push_back(CAoDcoinSecret(vchSecret, fCompressed).ToString());
            params.push_back("AoD");
            try
            {
                string result = CallAoDcoinRPC("importprivkey", params);
