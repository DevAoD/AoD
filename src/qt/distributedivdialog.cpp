...
const char* DistributeDivDialog::columnHeadings[] = {
    "AoD Address", "AoD", "AoDcoin Address", "Dividend"
};
...
void DistributeDivDialog::on_calcDividendsButton_clicked()
{
    ...
    for (DistributionVector::const_iterator it = vDistribution.begin(); it != vDistribution.end(); it++, i++)
    {
        ...
        model->setItem(i, 2, new QAddressItem(it->GetAoDcoinAddress()));
...
void DistributeDivDialog::on_exportButton_clicked()
{
    ...
    for (unsigned int i=0; i < vDistribution.size(); i++)
    {
        fprintf(fp, "%s,%lld,%s,%f\n",
                ...
                vDistribution[i].GetAoDcoinAddress().ToString().c_str(),
...
bool DistributeDivDialog::ConfirmDistribution()
{
    ...
    {
        QMessageBox::critical(this, "Error", QString("Unable to get AoDcoin balance: %1").arg(error.what()));
        return false;
    }
    QString sQuestion = QString("%1 AoDcoins will be sent to %2 addresses in %3 transaction(s).\nYour current AoDcoin balance is %4.\n\nAre you sure?").arg(
