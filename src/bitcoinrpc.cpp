...
extern Value exportAoDcoinkeys(const Array& params, bool fHelp);
...
// AoDcoin: get network Gh/s estimate
// AoD note: this is only useful during the initial proof-of-work 'IPO' phase of the network
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new AoD address for receiving payments.  "
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current AoD address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <AoDaddress> <account>\n"
            "Sets the account associated with the given address.");

    CBitcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid AoD address");
...
Value getAoDcoinaddresses(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getAoDcoinaddresses <account>\n"
            "Returns the list of addresses and the associated AoDcoin address for the given account.");
    ...
    {
        const CBitcoinAddress& address = item.first;
        const CAoDcoinAddress AoDcoinAddress(address);
        const string& strName = item.second;
        if (strName == strAccount)
            ret.push_back(Pair(address.ToString(), AoDcoinAddress.ToString()));
    }
    return ret;
}
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 4))
        throw runtime_error(
            "sendtoaddress <AoDaddress> <amount> [comment] [comment-to]\n"
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <AoDaddress> <message>\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <AoDaddress> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <AoDaddress> [minconf=1]\n"
            "Returns the total amount received by <AoDaddress> in transactions with at least [minconf] confirmations.");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 3 || params.size() > 6))
        throw runtime_error(
            "sendfrom <fromaccount> <toAoDaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001\n"
            "requires wallet passphrase to be set with walletpassphrase first");
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() < 3 || params.size() > 6))
        throw runtime_error(
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid AoD address:")+s.name_);
...
Value distribute(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "amount is the the number of AoDcoins to distribute, in double-precision floating point number.\n"
            "If proceed is not true the AoDcoins are not sent and the details of the distribution are returned.");
    ...
    else
    {
        ...
        {
            Object obj;
            obj.push_back(Pair("AoD_address", distribution.GetPeershareAddress().ToString()));
            obj.push_back(Pair("balance", (double)distribution.GetBalance() / COIN));
            obj.push_back(Pair("AoDcoin_address", distribution.GetAoDcoinAddress().ToString()));
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    // AoDcoin: if user OS account compromised prevent trivial sendmoney commands
    if (params.size() > 2)
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "Wallet encrypted; AoD server stopping. Please restart server to run with encrypted wallet";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <AoDaddress>\n"
            "Return information about <AoDaddress>.");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "AoD is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "AoD is downloading blocks...");
...
// AoDcoin: get information of sync-checkpoint
...
// AoDcoin: reserve balance from being staked for network protection
...
// AoDcoin: check wallet integrity
...
// AoDcoin: make a public-private key pair
...
{ //  name                      function                 safe mode?
  //  ------------------------  -----------------------  ----------
    ...
    { "getAoDcoinaddresses",   &getAoDcoinaddresses,   true },
    ...
    { "exportAoDcoinkeys",     &exportAoDcoinkeys,     false },
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: AoD-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
std::string CallAoDcoinRPC(const std::string &strMethod, const Array &params)
{
    if (mapAoDcoinArgs["-rpcuser"] == "" && mapAoDcoinArgs["-rpcpassword"] == "")
        throw runtime_error(strprintf(
            _("You must set rpcpassword=<password> in the AoDcoin configuration file:\n%s\n"
              "If the file does not exist, create it with owner-readable-only file permissions."),
                GetAoDcoinConfigFile().string().c_str()));
    ...
    SSLIOStreamDevice d(sslStream, fUseSSL);
    iostreams::stream<SSLIOStreamDevice> stream(d);
    if (!d.connect(GetAoDcoinArg("-rpcconnect", "127.0.0.1"), GetAoDcoinArg("-rpcport", CBigNum(fTestNet? AoDcoin_TESTNET_RPC_PORT : AoDcoin_RPC_PORT).ToString().c_str())))
        throw runtime_error("couldn't connect to AoDcoin RPC server");

    // HTTP basic authentication
    string strUserPass64 = EncodeBase64(mapAoDcoinArgs["-rpcuser"] + ":" + mapAoDcoinArgs["-rpcpassword"]);
    map<string, string> mapRequestHeaders;
    mapRequestHeaders["Authorization"] = string("Basic ") + strUserPass64;
    ...
    if (error.type() != null_type)
    {
        ...
        throw AoDcoin_rpc_error(nCode, sMessage);
    }
    else
    {
        // Result
