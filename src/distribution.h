...
{
protected:
    CBitcoinAddress addrAoD;
    int64 nBalance;

    CAoDcoinAddress addrAoDcoin;
    double dDividendAmount;

public:
    Distribution(CBitcoinAddress addrAoD, int64 nBalance, double dDividendAmount)
        : addrAoD(addrAoD), nBalance(nBalance), addrAoDcoin(addrAoD), dDividendAmount(dDividendAmount)
    {
    }
    ...
    const CAoDcoinAddress GetAoDcoinAddress() const
    {
        return addrAoDcoin;
    }
