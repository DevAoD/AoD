...
// Copyright (c) 2013-2014 AoD Developers
...
class CAoDcoinAddress : public CBitcoinAddress
{
public:
    enum
    {
        PUBKEY_ADDRESS = 55,  // AoDcoin: addresses begin with 'P'
        SCRIPT_ADDRESS = 117, // AoDcoin: addresses begin with 'p'
    ...
    CAoDcoinAddress(uint160 hash160In)
    {
        SetHash160(hash160In);
    }

    CAoDcoinAddress(const CBitcoinAddress &address)
    {
        if (address.IsScript())
...
class CAoDcoinSecret : public CBitcoinSecret
{
public:
    void SetSecret(const CSecret& vchSecret, bool fCompressed)
    {
        assert(vchSecret.size() == 32);
        SetData(128 + (fTestNet ? CAoDcoinAddress::PUBKEY_ADDRESS_TEST : CAoDcoinAddress::PUBKEY_ADDRESS), &vchSecret[0], vchSecret.size());
        if (fCompressed)
            vchData.push_back(1);
    }
    ...
    bool IsValid() const
    {
        ...
        switch(nVersion)
        {
             case (128 + CAoDcoinAddress::PUBKEY_ADDRESS):
                break;

            case (128 + CAoDcoinAddress::PUBKEY_ADDRESS_TEST):
                fExpectTestNet = true;
                break;
    ...
    CAoDcoinSecret(const CSecret& vchSecret, bool fCompressed)
    {
        SetSecret(vchSecret, fCompressed);
    }
};
